...
for k=1:size(Schraffuren,1)
    if ~isempty(Schraffuren{k,1})
        ...
        if floor(PSOArray{k,1}(1,1)/2^31)<floor(PSOArray{k,1}(end,1)/2^31)
            disp(['Ebene: ',int2str(k),' wird wegen Aerotechproblemen behandelt']);
            %PSOArray{k,1}=[];
            %PSOEntry(k,1)=0;
            %Nötige Umdrehungen für Aufschiebung werden berechnet und aufaddiert
            uebergang=floor(PSOArray{k,1}(end,1)/2^31)*2^31;
            differenz=uebergang-PSOArray{k,1}(1,1); 
            umdrehungen=ceil(differenz/100000);
            PSOArray{k,1}(:,1)=PSOArray{k,1}(:,1)+umdrehungen*100000;
            for j=k:size(Schraffuren,1) 
                Schraffuren{j,1}(:,2)=Schraffuren{j,1}(:,2)+umdrehungen*360;
            end
        end
    end
end
...
fprintf(fid,['DVAR $global[',int2str(AnzahlPSOEntry),']','\r\n']);
%PSOArray in file ausschreiben
PSOArrayString=['$global[','%1.0f',']=','%1.0f','\r\n'];
PSOArrayIndex=0;
...
fprintf(fid,['ARRAY A WRITE $global[0] 0 ',int2str(AnzahlPSOEntry),'\r\n']);
fprintf(fid,['\r\n']);
